...
public class Calculo {
   ...
   static double SumaDeRienman(int a, int nIntervalos){
        ...
        double IntegralApprox = sumatoria(x, y, z, 0.0);
   ...
   static double sumatoria(double[] x, double[] y, double z[], double IntegralApprox){
        for(double xi: x){
            for (double yi: y){
                for (double zi: z){
                    double resultado = 1/ Math.pow(xi*xi + yi*yi + zi * zi + 1, 2);
